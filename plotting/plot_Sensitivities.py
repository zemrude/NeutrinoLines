...
def extractMedianLimit(LECut,HECut,mode, channel, syst, profile, m_min, m_max, binning1, binning2, oversampling=100, CL='_90CL',test=''):
    ...
    for m in all_masses:
        ...
        resultFile = base_path+'sensitivity/'+mode+'/'+channel+'/'+syst+'/'+test+'Sensitivity_effective_'+syst+'_'+mode+'_'+profile+'profile_LEBDT'+str(LECut)+'_HEBDT'+str(HECut)+'_'+channel+'_'+str(int(m))+'GeV_binsmerged_'+str(binning1)+'-'+str(binning2)+'_oversampling'+str(oversampling)+CL+'.npy'
        try: 
            results = np.load(resultFile).item()
            x.append(results['mass'])
            y.append(results['median'])
        except:
            print 'problem with file',resultFile[len(basepath):]
    return x,y    
...
def extractBrazilLimit(LECut,HECut,mode, channel,syst, profile, m_min, m_max, binning1, binning2, oversampling=100, CL='_90CL'):
    ...
    for m in all_masses:
        ...
        resultFile = base_path+'sensitivity/'+mode+'/'+channel+'/'+syst+'/Sensitivity_effective_'+syst+'_'+mode+'_'+profile+'profile_LEBDT'+str(LECut)+'_HEBDT'+str(HECut)+'_'+channel+'_'+str(int(m))+'GeV_binsmerged_'+str(binning1)+'-'+str(binning2)+'_oversampling'+str(oversampling)+CL+'.npy'
        try: 
            results = np.load(resultFile).item()
            x.append(results['mass'])
...
def plot_results(mode, channel, profile):
    ...
    if mode == 'annihilation':
        ...
        for ex in ['Fermi','ANTARES','ICDC','IC86']:
...
c='nue'
sens_1[c] = extractMedianLimit(0.15,0.2,'annihilation',c,'nominal','NFW',30,900, 2, 5, 100, '_90CL')
sens_2[c] = extractMedianLimit(-1.0,0.3,'annihilation',c,'nominal','NFW',900,50000, 2, 5, 200, '_90CL')    
sens_combined[c] = []
...
for c in channels:
    sens_1[c] = extractMedianLimit(0.15,0.2,'annihilation',c,'nominal','NFW',30,900, 2, 5, -1, '_90CL')
    sens_2[c] = extractMedianLimit(-1.0,0.3,'annihilation',c,'nominal','NFW',900,50000, 2, 5, -1, '_90CL')    
    sens_combined[c] = []
    sens_combined[c].append(np.append(np.array(sens_1[c][0]),np.array(sens_2[c][0])))
    sens_combined[c].append(np.append(np.array(sens_1[c][1]),np.array(sens_2[c][1])))
...
plt.text(12*1e3, 1e-21/2, 'NFW profile',fontsize=17, color = 'black')

leg = ax.legend(frameon = 1, fancybox=False, loc='lower left', bbox_to_anchor=(-0.0175, 1., 1.035, 0),labelspacing=0.2, mode="expand", ncol=1)
fig.tight_layout()
fig.savefig('plots/Sensitivity_annihilation_channels_NFW.pdf',bbox_inches = 'tight') 
fig.savefig('plots/Sensitivity_annihilation_channels_NFW.png',bbox_inches = 'tight') 
...
c='nue'
sens_1[c] = extractMedianLimit(0.15,0.2,'decay',c,'nominal','NFW',30,900, 2, 5, 100, '_90CL')
sens_2[c] = extractMedianLimit(-1.0,0.3,'decay',c,'nominal','NFW',900,50000, 2, 5, 100, '_90CL')    
sens_combined[c] = []
sens_combined[c].append(np.append(np.array(sens_1[c][0]),np.array(sens_2[c][0])))
sens_combined[c].append(np.append(np.array(sens_1[c][1]),np.array(sens_2[c][1])))
...
for c in channels:
    sens_1[c] = extractMedianLimit(0.15,0.2,'decay',c,'nominal','NFW',30,900, 2, 5, -1, '_90CL')
    sens_2[c] = extractMedianLimit(-1.0,0.3,'decay',c,'nominal','NFW',900,50000, 2, 5, -1, '_90CL')    
    sens_combined[c] = []
    sens_combined[c].append(np.append(np.array(sens_1[c][0]),np.array(sens_2[c][0])))
    sens_combined[c].append(np.append(np.array(sens_1[c][1]),np.array(sens_2[c][1])))
...
plt.text(12*1e3, 1e29/2, 'NFW profile',fontsize=17, color = 'black')

leg = ax.legend(frameon = 1, fancybox=False, loc='lower left', bbox_to_anchor=(-0.0175, 1., 1.035, 0),labelspacing=0.2, mode="expand", ncol=1)
fig.tight_layout()
fig.savefig('plots/Sensitivity_decay_channels_NFW.pdf',bbox_inches = 'tight') 
fig.savefig('plots/Sensitivity_decay_channels_NFW.png',bbox_inches = 'tight') 
