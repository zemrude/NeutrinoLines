...
scrambled_outfile_path = base_path + 'PDFs/ScrambledSignal/' + mode
outfile_path = base_path + 'PDFs/Signal/' + mode
...
scrambled_filename_template =  'PDF_DM_FullSkyScrambled_' + template
filename_template =  'PDF_DM_' + template

outfile = outfile_path+filename_template+'.pkl'
outfile_quad = outfile_path+filename_template+'_quad.pkl'

scrambled_outfile = scrambled_outfile_path + scrambled_filename_template+'.pkl'
scrambled_outfile_quad = scrambled_outfile_path + scrambled_filename_template + '_quad.pkl'

print (" Signal PDF outfile : %s "%outfile)
print (" Scrambled PDF outfile : %s "%scrambled_outfile)
...
for fileType in allFiles['MC'][systematics].keys():
    ...
    text = "  Opening file %s"%inputfile.split('/')[-1]
    ...
    signal_psi_reco = np.append(signal_psi_reco, psi_reco)
    signal_weight = np.append(signal_weight,new_weight)     
    
    
    #Now we scrambled, we take a random RA from 0 - 2pi
    scrambled_RA_reco = np.random.random_sample((len(new_dec_reco),)) * 2*np.pi
